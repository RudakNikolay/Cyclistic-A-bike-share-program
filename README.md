...
Проект нацелен на улучшение коммуникации с потребителями, лучшему пониманию из потребностей, выстраиванию маркетинговой политики для привлечения новых клиентов. Так же важным параметром является сохранение и увеличение райдеров с подпиской.

Проект полностью выполнен с использованием R-studio, версия 2023.03.0+386.
...
```
Данные для анализа доступны по [ссылке.](https://divvy-tripdata.s3.amazonaws.com/index.html)  
Загрузка данных в R с использование функции read_csv пакета readr включенного в tidyverse.  
...
```
Создав дата-сет трансформирую его, дабавляем две колонки с помощью функции mutate.  
...
```
С помощью функции filter очищаю дата-сет от записей с временем поездки меньше минуты и от записей где не указаны координаты конца поездки.
```
df<- filter (df, ride_length > 0,!is.na(end_lat))
```
В разрезе типа велосипеда и велосипедиста (подписчик / случайный райдер) рассчитываю время всех поездок в секундах, время средней поездки и общее количество поездок. Аналогичные данные высчитываю в разрезе дней недели. 
...
Данные получены. Приступаю к построению графиков основанных на данных с помощью пакета ggplot2.  

Построил графики отражающие среднее время одной поездки в минутах и время проката
```
